...
export function ChartSkeleton({ className }: { className?: string }) {
  return (
    // Poistettu h-full, lisätty oletuskorkeus tai className-pohjainen korkeus
    <div className={`rounded-2xl border border-slate-100 bg-white p-6 shadow-sm animate-pulse ${className || "h-[300px]"}`}>
      <div className="h-4 w-1/3 bg-slate-100 rounded mb-6" />
      <div className="h-[200px] w-full bg-slate-50 rounded" />
    </div>
  );
}
...
export function ListSkeleton({ rows = 5 }: { rows?: number }) {
  return (
    <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm animate-pulse">
      <div className="h-6 w-1/4 bg-slate-100 rounded mb-6" />
