    ...
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
        {[...Array(6)].map((_, i) => (
          <div
            key={i}
            className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 flex items-center gap-4 animate-pulse shadow-sm"
          >
            <div className="h-12 w-12 rounded-xl bg-slate-100 dark:bg-slate-800 shrink-0" />
            <div className="space-y-2 w-full">
              <div className="h-6 w-8 bg-slate-200 dark:bg-slate-700 rounded-md" />
              <div className="h-4 w-16 bg-slate-100 dark:bg-slate-800 rounded-md" />
            </div>
          </div>
        ))}
      </div>
  ...
  const stats = [
    {
      label: "Yhteensä",
      value: total,
      icon: <Briefcase className="h-5 w-5 text-slate-600 dark:text-slate-400" />,
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
      {stats.map((stat, i) => (
        <div
          key={i}
          className={`bg-white dark:bg-slate-900 border ${stat.border} rounded-2xl p-4 flex items-center gap-4 shadow-sm hover:shadow-md transition-shadow`}
        >
          <div className={`p-3 rounded-xl ${stat.bg}`}>{stat.icon}</div>
          <div>
            <p className="text-2xl font-bold text-slate-900 dark:text-slate-50 leading-none mb-1">
              {stat.value}
            </p>
